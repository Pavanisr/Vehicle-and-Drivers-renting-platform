...
export default function BookingRequests() {
  ...
  const updateStatus = async (id: number, status: string) => {
    try {
      ...
      await axios.put(
        `http://localhost:3000/api/drivers/requests/${id}/status`,
        { status },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      setBookings((prev) =>
        prev.map((b) => (b.booking_id === id ? { ...b, status } : b))
      );
    } catch (err) {
      console.error(err);
    ...
    <div className="container py-5" style={{ minHeight: "100vh", backgroundColor: "#eef5ff" }}>
      ...
      <div className="mb-4 d-flex align-items-center">
        ...
        <div className="text-center w-100">
          ...
          <p className="text-secondary">Manage your customer booking requests</p>
        </div>
      </div>
